 ...
 */ async function generateDynamicFlightRenderResult(req, ctx, requestStore, options) {
    const { clientReferenceManifest, componentMod: { renderToReadableStream, createElement }, htmlRequestId, renderOpts, requestId, workStore } = ctx;
    ...
    if (process.env.NODE_ENV === 'development' && isBypassingCachesInDev(renderOpts, requestStore)) {
 ...
 */ async function generateDynamicFlightRenderResultWithCachesInDev(req, ctx, initialRequestStore, createRequestStore) {
    const { htmlRequestId, renderOpts, requestId, workStore } = ctx;
    const { dev = false, onInstrumentationRequestError, setReactDebugChannel, setCacheStatus } = renderOpts;
    ...
    const onError = (0, _createerrorhandler.createFlightReactServerErrorHandler)(dev, onFlightDataRenderError);
    const getPayload = (requestStore)=>_workunitasyncstorageexternal.workUnitAsyncStorage.run(requestStore, generateDynamicRSCPayload, ctx, undefined);
    const { stream, debugChannel } = await renderWithRestartOnCacheMissInDev(ctx, initialRequestStore, createRequestStore, getPayload, onError);
...
function querystringEncode(query) {
    ...
    if (isStaticGeneration) {
        ...
        if (isRSCRequest) {
            if (isRuntimePrefetchRequest) {
                ...
                const bypassCachesInDev = isBypassingCachesInDev(renderOpts, requestStore);
                if (process.env.NODE_ENV === 'development' && process.env.NEXT_RUNTIME !== 'edge' && cacheComponents && !bypassCachesInDev) {
                    return generateDynamicFlightRenderResultWithCachesInDev(req, ctx, requestStore, createRequestStore);
                } else {
                    // Set cache status to bypass when specifically bypassing caches in dev
                    if (process.env.NODE_ENV === 'development' && bypassCachesInDev && renderOpts.setCacheStatus) {
...
async function renderToStream(requestStore, req, res, ctx, tree, formState, postponedState, metadata, createRequestStore, devValidatingFallbackParams) {
    ...
    try {
        ...
        cacheComponents && // We only do this flow if we can safely recreate the store from scratch
        ...
        !isBypassingCachesInDev(renderOpts, requestStore)) {
            ...
            const { stream: serverStream, debugChannel, requestStore: finalRequestStore } = await renderWithRestartOnCacheMissInDev(ctx, requestStore, createRequestStore, getPayload, serverComponentsErrorHandler);
...
function createAsyncApiPromisesInDev(stagedRendering, cookies, mutableCookies, headers) {
    return {
        ...
        sharedParamsParent: stagedRendering.delayUntilStage(_stagedrendering.RenderStage.Runtime, 'params', '<internal params>'),
        sharedSearchParamsParent: stagedRendering.delayUntilStage(_stagedrendering.RenderStage.Runtime, 'searchParams', '<internal searchParams>'),
