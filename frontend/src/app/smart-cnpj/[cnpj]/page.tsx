...
export default function CompanyDetailsPage({ params }: CompanyDetailsPageProps) {
  ...
  const getBackLink = () => {
    switch (from) {
      ...
      default:
        // Construir URL de volta com parâmetros de busca preservados
        if (returnType && returnQuery) {
          return { 
            href: `/smart-cnpj/results?type=${returnType}&q=${encodeURIComponent(returnQuery)}`, 
        ...
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-gray-600 max-w-md mx-auto">
              Buscando informações do CNPJ <strong>{smartCNPJService.formatCNPJ(cnpj)}</strong>
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-gray-600 max-w-md mx-auto mb-6">
              {error 
                ? 'Ocorreu um erro ao buscar os dados da empresa. Tente novamente.'
                : `Não foi possível encontrar uma empresa com o CNPJ ${smartCNPJService.formatCNPJ(cnpj)}.`
              }
            </p>
